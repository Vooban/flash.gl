...
function coordinatesContainPoint(coordinates, point) {
  ...
  for (let i = -1, l = coordinates.length, j = l - 1; ++i < l; j = i) {
    if (
      ...
      point[0] <
        ((coordinates[j][0] - coordinates[i][0]) * (point[1] - coordinates[i][1])) /
...
export function arcgisToGeoJSON(arcgis, idAttribute, webMercatorUtils) {
  let geojson = {};

  if (arcgis.features) {
    geojson.type = 'FeatureCollection';
    geojson.features = arcgis.features.map((f) =>
      arcgisToGeoJSON(f, idAttribute, webMercatorUtils)
    );
  }

  if (typeof arcgis.x === 'number' && typeof arcgis.y === 'number') {
    geojson.type = 'Point';
    geojson.coordinates = webMercatorUtils.xyToLngLat(arcgis.x, arcgis.y);
  ...
  if (arcgis.paths) {
    if (arcgis.paths.length === 1) {
      ...
      geojson.coordinates = arcgis.paths[0]
        .slice(0)
        .map((c) => c.map((cc) => webMercatorUtils.xyToLngLat(cc[0], cc[1])));
    } else {
  ...
  if (arcgis.geometry || arcgis.attributes) {
    ...
    geojson.geometry = arcgis.geometry
      ? arcgisToGeoJSON(arcgis.geometry, null, webMercatorUtils)
      : null;
  ...
  if (
    ...
    console.warn(
      `Object converted in non-standard crs - ${JSON.stringify(arcgis.spatialReference)}`
    );
  }
